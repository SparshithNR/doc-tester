...
function getCode(codeArray, importsArray) {
  ...
  codeArray.forEach((element) => {
    ...
    if (assertion) {
      fileContent += getAssertion(assertion.groups);
      assertCount++;
    }
  });
  ...
  /* const ast = parse(fileContent, {allowImportExportEverywhere: true});
  let generatedCode = generate(ast, {}, fileContent); */
...
function getAssertionComponenets(element) {
  return element.match(/(?<code>.*)\/\/\s(?<assertion>\w+)(:?:\s(?<expected>.+))?;/);
}
