...
async function executeTests(inspect) {
  let args = [];
  if (inspect) {
    console.log(`Make sure you added connection in node dev tool for port ${inspect}`);
    args.push(`--inspect-brk=${inspect}`);
  }
  args.push.apply(args, [`node_modules/.bin/qunit`, `test.js`]);
